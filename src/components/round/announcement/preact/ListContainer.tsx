  ...
  return (
    <>
      <table class="table-base custom-responsive-table-md table-pin-rows table">
        <thead>
          <tr>
            <th class="text-center" title="分類">
              分類
            </th>
            <th class="w-2/5 text-center" title="主旨">
              主旨
            </th>
        ...
        <tbody>
          {$items.map(({ _id, category, createdAt, voided, subject, creator }) => (
            <tr key={_id} class="*:px-2">
              <td class="text-center" data-title="分類">
                {categoryDisplayName(category)}
              </td>
              <td class="text-left" data-title="主旨">
